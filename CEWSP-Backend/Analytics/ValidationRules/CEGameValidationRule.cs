...
{
    public class CEGameValidationRule : BaseValidationRule
    {
        private CERootPathValidationRule m_rootPathValidationRule;
        private CEPathValidationRule m_pathValidationRule;
        private string m_sCERootPath;
        private string m_sPathToCheck;
        private CEVersionInfo m_versionInfo;
        ...
        public override bool IsValid(object o, ref ReasonList reasons, params object[] additionalArgs)
        {
            ...
            if (additionalArgs.Count() != 2)
                throw new ArgumentException("additionalArg does not contain the expected CE root path and CEVersion");

            if (additionalArgs[0] is string == false)
                throw new ArgumentException("additionalArgs element 0 is not a string");

            if (additionalArgs[1] is CEVersionInfo == false)
                throw new ArgumentException("additionalArgs element 1 is not of the type CEVersionInfo");

            m_sCERootPath = additionalArgs[0] as string;
            m_sPathToCheck = o as string;
            m_versionInfo = additionalArgs[1] as CEVersionInfo;

            // Check root folder first
            if (!m_rootPathValidationRule.IsValid(m_sCERootPath, m_versionInfo))
            {
                var r = new Reason()
                {
